...
func renderTranscript(
	...
	for _, message := range messages {
		...
		case gogent.MessageRoleAssistant:
			...
			for i, toolCall := range message.ToolCalls {
				...
				if result, ok := toolResults[toolCall.ID]; ok {
					b.WriteByte('\n')
					b.WriteString(renderToolResultMessage(result, width))
		...
		case gogent.MessageRoleTool:
			...
			writeTranscriptGap(&b, &first)
			b.WriteString(renderToolResultMessage(message, width))
...
func renderToolResultMessage(message gogent.Message, width int) string {
